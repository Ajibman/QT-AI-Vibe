...
def main():
    ...
    p.add_argument("--owner", default="Ajibman", help="Repo owner")
    p.add_argument("--repo", default="QT-AI-Vibe", help="Repo name")
    p.add_argument("--keystore-file", default=os.environ.get("KEYSTORE_PATH"), help="Path to keystore file")
    p.add_argument("--keystore-password", default=os.environ.get("KEYSTORE_PASSWORD"), help="Keystore password")
    p.add_argument("--key-alias", default=os.environ.get("KEY_ALIAS"), help="Key alias")
    p.add_argument("--key-password", default=os.environ.get("KEY_PASSWORD"), help="Key password")
    args = p.parse_args()
    ...
    if args.keystore_file:
        ...
        print("Skipping", SECRET_NAMES["keystore"])

    if args.keystore_password:
        to_upload[SECRET_NAMES["keystore_password"]] = args.keystore_password.encode()
    else:
        print(f"Warning: {SECRET_NAMES['keystore_password']} will not be uploaded.")

    if args.key_alias:
        to_upload[SECRET_NAMES["key_alias"]] = args.key_alias.encode()
    else:
        print(f"Warning: {SECRET_NAMES['key_alias']} will not be uploaded.")

    if args.key_password:
        to_upload[SECRET_NAMES["key_password"]] = args.key_password.encode()
    else:
        print(f"Warning: {SECRET_NAMES['key_password']} will not be uploaded.")

    if not to_upload:
        print("No secrets to upload.", file=sys.stderr)
        sys.exit(1)
